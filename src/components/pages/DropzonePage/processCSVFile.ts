...
export const processCsvFile = async (extFile: ExtFile): Promise<SimpleMetadataRow[]> => {
  if (!extFile.file) {
  ...
  try {
    ...
    const metadataRows: SimpleMetadataRow[] = metadataObjects.map((metadata) => ({ metadata, fileName: extFile.name! }));
